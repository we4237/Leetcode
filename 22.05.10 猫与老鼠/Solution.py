...
class Solution:
    def catMouseGame(self, graph: List[List[int]]) -> int:
        ...
        for i in range(1,n):
            results[i][i][MOUSE_TURN]=MOUSE_WIN
        ...
        while q:
            ...
            if turn == MOUSE_TURN:
                preStates = [(mouse,prev,CAT_TURN) for prev in graph[cat]]
            else:
                preStates=[(prev,cat,MOUSE_TURN) for prev in graph[mouse]]
            for prevMouse,prevCat,prevTurn in preStates:
                if prevCat == 0:
                    continue
